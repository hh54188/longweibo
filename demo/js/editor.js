...
(function (global) {
	...
	var ftConvert = function (cc) {
	    var str = '';
	    for (var i = 0; i < cc.length; i++) {
	        if (charjt().indexOf(cc.charAt(i)) != -1) str += charft().charAt(charjt().indexOf(cc.charAt(i)));
	        else if (charhx().indexOf(cc.charAt(i)) != -1) str += charft().charAt(charhx().indexOf(cc.charAt(i)));
	...
	global.convertToft = function () {
		var text = $('#cke_ckeditor1 iframe').contents().find('.cke_show_borders').html();
		var arr = [];
		var str = "";
		for (var i = 0; i < text.length; i++) {
			var temp = ftConvert(text[i]); 
			str = str + temp;
		}	

		$('#cke_ckeditor1 iframe').contents().find('.cke_show_borders').html(str);
	}

	global.convertTojt = function () {
		var text = $('#cke_ckeditor1 iframe').contents().find('.cke_show_borders').html();
		var arr = [];
		var str = "";
		for (var i = 0; i < text.length; i++) {
			var temp = jtConvert(text[i]); 
			str = str + temp;
		}	
	...
	var config = {
		font_names:"微软雅黑/'微软雅黑', Arial/Arial, Helvetica, sans-serif;Comic Sans MS/Comic Sans MS, cursive;Courier New/Courier New, Courier, monospace;Georgia/Georgia, serif;Lucida Sans Unicode/Lucida Sans Unicode, Lucida Grande, sans-serif;Tahoma/Tahoma, Geneva, sans-serif;Times New Roman/Times New Roman, Times, serif;Trebuchet MS/Trebuchet MS, Helvetica, sans-serif;Verdana/Verdana, Geneva, sans-serif",
		height: '300px',
	...
	global.init = function () {
		$('#ckeditor1').ckeditor(config).ckeditor(function () {

		});
	}

	global.shutEditArea = function () {
		...
		$('#cke_ckeditor1 iframe').contents().find('.cke_show_borders').empty();
	}

	global.openEditArea = function () {
		$('#overlay').add('#ckeditor-wrap').fadeIn();
	}

	global.setImgSrc = function (src) {
		$('#preview-img').attr('src', src).fadeIn();
	}	

	global.myScreenShot = function () {
		...
		var screenshot = window.frames["screen-shot-frame"].screenshot;
		screenshot();				
	}
})(myEditor)